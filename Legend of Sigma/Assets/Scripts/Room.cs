...
public class Room : MonoBehaviour
{
    ...
    public void SetupRoom(IntRange widthRange, IntRange heightRange, int columns, int rows, Corridor corridor, GameObject enemy, bool first)
    {
        ...
        if (!first)
        {
            IntRange wRand = new IntRange(1, roomWidth);
            IntRange hRand = new IntRange(1, roomHeight);
            for (int j = 0; j < 2; j++)
            {
                Vector3 pos = new Vector3(wRand.Random + xPos, hRand.Random + yPos, -1);
